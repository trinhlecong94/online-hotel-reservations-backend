...
public interface PromoRepository extends JpaRepository<PromoEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM promo WHERE start_Date <= NOW() and end_Date >= NOW()", nativeQuery = true)
    List<PromoEntity> getAllPromoStillActive();

    @Query(value = "SELECT * FROM promo WHERE start_Date <= NOW() and end_Date >= NOW() AND promo.promo_code=:code", nativeQuery = true)
